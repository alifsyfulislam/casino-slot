...
var ITEM_COUNT = 4 // item count in slots
    ...
    preloadImages( items, function() {
        ...
        function _fill_canvas( canvas, items ) {
            ...
            for (var i = 0 ; i < ITEM_COUNT ; i++) {
                ...
                ctx.shadowOffsetY = 5;
                ctx.shadowBlur = 5;
                ctx.drawImage(asset.img, 1, i * SLOT_HEIGHT + IMAGE_TOP_MARGIN);
                ctx.drawImage(asset.img, 1, (i + ITEM_COUNT) * SLOT_HEIGHT + IMAGE_TOP_MARGIN);
                ctx.restore();
                ctx.fillRect(0, i * SLOT_HEIGHT, 74, SLOT_SEPARATOR_HEIGHT);
                ctx.fillRect(0, (i + ITEM_COUNT)  * SLOT_HEIGHT, 74, SLOT_SEPARATOR_HEIGHT);
            }
        }
...
Game.prototype.restart = function() {
    ...
    if (x==1 || x==2 || x==3) {
        this.result1 = _find( this.items1, 'fridge' );
        this.result2 = _find( this.items2, 'fridge' );
        this.result3 = _find( this.items3, 'fridge' );
    }
...
Game.prototype.update = function() {
    ...
    switch (this.state) {
        ...
        function _add_win( delay, score ) {
            ...
            $('#results').show();
            if (that.items1[that.result1].id == 'fridge') {
                ec++;
            }
            if (that.items2[that.result2].id == 'fridge') {
                ec++;
            }
            if (that.items3[that.result3].id == 'fridge') {
                ec++;
            }
            $('#multiplier').text(ec);
